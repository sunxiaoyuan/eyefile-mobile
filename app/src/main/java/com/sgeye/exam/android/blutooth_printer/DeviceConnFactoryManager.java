        ...
        public void handleMessage(Message msg) {
            switch (msg.what) {
                case READ_DATA:
                    ...
                    String status = Margaret.getApplicationContext().getString(R.string.str_printer_conn_normal);
                    if (sendCommand == esc) {
                        //设置当前打印机模式为ESC模式
                        if (currentPrinterCommand == null) {
                            ...
                            if (result == 0) {//打印机状态查询
                                ...
                                Margaret.getApplicationContext().sendBroadcast(intent);
                            } else if (result == 1) {//查询打印机实时状态
                                if (whichFlag) {
                                    if ((buffer[0] & ESC_STATE_PAPER_ERR) > 0) {
                                        status += " " + Margaret.getApplicationContext().getString(R.string.str_printer_out_of_paper);
                                    }
                                    if ((buffer[0] & ESC_STATE_COVER_OPEN) > 0) {
                                        status += " " + Margaret.getApplicationContext().getString(R.string.str_printer_open_cover);
                                    }
                                    if ((buffer[0] & ESC_STATE_ERR_OCCURS) > 0) {
                                        status += " " + Margaret.getApplicationContext().getString(R.string.str_printer_error);
                                    }
                                } else {
                                    if ((buffer[0] == ESC_LOW_POWER)) {
                                        status = Margaret.getApplicationContext().getString(R.string.str_low_power);
                                    } else if ((buffer[0] == ESC_AMID_POWER)) {
                                        status = Margaret.getApplicationContext().getString(R.string.str_amid_power);
                                    } else if ((buffer[0] == ESC_HIGH_POWER)) {
                                        status = Margaret.getApplicationContext().getString(R.string.str_high_power);
                                    } else if ((buffer[0] == ESC_CHARGING)) {
                                        status = Margaret.getApplicationContext().getString(R.string.str_charging);
                                    }
                                }

                                System.out.println(Margaret.getApplicationContext().getString(R.string.str_state) + status);
                                String mode = Margaret.getApplicationContext().getString(R.string.str_printer_printmode_esc);
                                Utils.toast(Margaret.getApplicationContext(), mode + " " + status);
                            }
                        }
                        ...
                        if (currentPrinterCommand == null) {
                            ...
                            if (cnt == 1) {//查询打印机实时状态
                                ...
                                System.out.println(Margaret.getApplicationContext().getString(R.string.str_state) + status);
                                String mode = Margaret.getApplicationContext().getString(R.string.str_printer_printmode_tsc);
                                Utils.toast(Margaret.getApplicationContext(), mode + " " + status);
                            } else {//打印机状态查询
                                Intent intent = new Intent(ACTION_QUERY_PRINTER_STATE);
                                intent.putExtra(DEVICE_ID, id);
                        ...
                        if (currentPrinterCommand == null) {
                            ...
                            if (cnt == 1) {
                                ...
                                String mode = Margaret.getApplicationContext().getString(R.string.str_printer_printmode_cpcl);
                                Utils.toast(Margaret.getApplicationContext(), mode + " " + status);
                            } else {//打印机状态查询
                                Intent intent = new Intent(ACTION_QUERY_PRINTER_STATE);
                                intent.putExtra(DEVICE_ID, id);
